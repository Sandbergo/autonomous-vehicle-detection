...
from ssd.modeling.backbone.basicres import BasicResnetModel
from ssd.modeling.backbone.resnet import resnet_base
...
def build_backbone(cfg):
    ...
    if backbone_name == "resnet":
        model = resnet_base(depth = 101, width=1, pretrained=True)
        return model
    if backbone_name == "basicres":
        model = BasicResnetModel(cfg)
        return model
